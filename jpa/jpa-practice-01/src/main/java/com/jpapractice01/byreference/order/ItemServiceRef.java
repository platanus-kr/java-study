...
public class ItemServiceRef {
    ...
    @Transactional
    public void appendItem(long orderId, ItemRef itemRef) {
        OrdersRef findOrder = orderRepositoryRef.findById(orderId).orElseThrow(IllegalArgumentException::new);
        findOrder.addItem(itemRef);
    }

    @Transactional
    public List<ItemRef> retrieveOrderItems(long orderId) {
        /**
        ...
        List<ItemRef> itemRefs = ordersRef.getItems();
        itemRefs.size(); // DTO 변환 등등의 작업을 위해 컬렉션에 접근하여 엔티티 명시 (컬렉션 접근)
        return itemRefs;
    }
